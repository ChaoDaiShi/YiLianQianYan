...
import { acceptPreferences, defaultPreferences, validPreferences, type ProductPreferences } from "./productPreferences";
...
export function useProductPreferences() {
  ...
  useEffect(() => {
    let active = true;
    void getProductPreferences().then((value) => { if (active) { setPreferences((current) => acceptPreferences(current, value)); setLoaded(true); } })
      .catch(() => { if (active) setError("配置服务不可用，已保留安全默认布局。"); });
    const receive = (event: Event) => setPreferences((current) => acceptPreferences(current, (event as CustomEvent).detail));
  ...
  const save = async (next: ProductPreferences) => {
    ...
    try {
      ...
      setError(""); setLoaded(true); setPreferences((current) => acceptPreferences(current, saved));
