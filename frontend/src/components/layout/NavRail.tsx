...
export default function NavRail() {
  ...
  return (
    <nav
      aria-label="全局导航"
      className="z-20 flex h-full w-[68px] shrink-0 flex-col items-center gap-0.5 border-r border-[var(--border)] bg-[var(--sidebar-bg)] py-3 text-[var(--sidebar-fg)] min-[960px]:w-[88px]"
    >
      <button
        type="button"
        onClick={() => navigate("/chat")}
        className="mb-2 shrink-0 rounded-xl p-1 transition-colors hover:bg-white/10"
        title="忆涟千言 · 新对话"
        aria-label="忆涟千言 · 新对话"
      >
        <img
      ...
      <div className="mb-2 w-8 shrink-0 border-t border-white/15" />
      ...
      <div className="scrollbar-thin min-h-0 w-full flex-1 overflow-y-auto overflow-x-hidden">
        {NAV_GROUPS.map((group, groupIndex) => (
          ...
          >
            ...
            <div className="mb-1 text-[9px] tracking-[0.18em] text-[var(--sidebar-muted)]">
              {group.label}
            </div>
                  ...
                  className={({ isActive }) =>
                    cn(
                      ...
                      isActive
                        ...
                        : "text-[var(--sidebar-fg)] opacity-80 hover:bg-white/10 hover:opacity-100",
                    )
                  }
                  title={item.label}
                >
      ...
      <div
        className="mb-1 mt-2 flex shrink-0 flex-col items-center gap-1 text-[9px] text-[var(--sidebar-fg)] opacity-80"
        aria-live="polite"
