...
| R6 | Capability center, imports and controlled improvement | Capability registry/pages, `capability/import_*`, protected import routes, managed Skill runtime owner | Current RC import/archive/CAS/rollback/Skill-owner tests and protected Capability page E2E passed | H5 Markdown Skill import → preview → confirm → disabled → enable/discover/use → disable/rollback | HUMAN_PENDING | Complete the H5 managed import lifecycle; declarative Plugin code remains inert by design. |
...
| R8 | Memory-to-Skill workflow | `backend/src/memory_skill.rs`, `api/skill_candidates.rs`, candidate/version UI and managed Skill discovery | Current RC authorization/sensitivity/validation/version/rollback/discovery suite passed | H5 completed task → candidate/source review/edit/confirm → managed version → later real reuse | HUMAN_PENDING | Complete one authorized candidate-to-reuse interaction; Skill file creation alone is insufficient. |
| R9 | Release quality | v1 profile, Tauri host, v1 migration registry, build configuration and release docs | Full frontend/Rust/Tauri/migration/E2E gate, boundary scans, RC build/hash and packaged GUI automation passed | Installer, shutdown/recovery and lightweight-device observation | BLOCKED | Current PAT cannot upload the `v1/**` Actions trigger without `workflow` scope; complete that maintainer step, then V1-H H6. Installer remains an unsigned RC, not a formal Release. |
